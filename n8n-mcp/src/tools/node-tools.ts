  ...
  execute: async (args: {
    ...
    try {
      ...
      const node = createWorkflowNode(
        args.node_type,
        args.node_name || nodeDef.displayName,
        args.position || [100, 100],
        args.parameters || {},
        args.typeVersion || nodeDef.version
      );
...
export const editNodeTool = {
  ...
  execute: async (args: {
    ...
    try {
      ...
      // Prepare updates
      const updates: any = {};
      if (args.node_name !== undefined) updates.name = args.node_name;
      if (args.position !== undefined) updates.position = args.position;
      if (args.parameters !== undefined) updates.parameters = args.parameters;
      if (args.typeVersion !== undefined) updates.typeVersion = args.typeVersion;
      if (args.webhookId !== undefined) updates.webhookId = args.webhookId;
      if (args.node_type !== undefined) updates.type = args.node_type;
...
export const deleteNodeTool = {
  ...
  execute: async (args: { workflow_name: string; node_id: string; workflow_path?: string }) => {
    try {
      // Load the workflow
      const workflow = await workflowService.loadWorkflow(args.workflow_name);
      if (!workflow) {
...
export const listAvailableNodesTool = {
  ...
  execute: async (args: {
    ...
    try {
      ...
      const nodeList = result.nodes.map((node, index) => 
        `${index + 1}. ${node.displayName} (${node.name})\n   Description: ${node.description}\n   Version: ${node.version}\n   Categories: ${node.codex?.categories?.join(', ') || 'None'}`
      ).join('\n\n');
...
export const getN8nVersionInfoTool = {
  ...
  execute: async (args: { random_string?: string }) => {
    try {
      ...
      return `n8n Version Information:\n\n` +
        ...
        `Subcategories:\n${Object.entries(categories.subcategories).map(([cat, subs]) => 
          `${cat}: ${subs.join(', ')}`
        ).join('\n')}`;
    } catch (error) {
