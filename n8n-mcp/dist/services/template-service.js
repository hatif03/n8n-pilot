...
export class TemplateService {
     ...
     */
    async listTemplates(limit = 10, offset = 0) {
    ...
    async searchTemplates(params) {
        ...
        if (params.query) {
            ...
            results = results.filter(template => template.name.toLowerCase().includes(query) ||
                template.description.toLowerCase().includes(query) ||
                template.author.name.toLowerCase().includes(query));
        }
        // Apply category filter
        if (params.categories && params.categories.length > 0) {
            results = results.filter(template => template.metadata?.categories?.some(cat => params.categories.includes(cat)));
        }
        // Apply complexity filter
        if (params.complexity) {
            results = results.filter(template => template.metadata?.complexity === params.complexity);
        }
        // Apply use case filter
        if (params.use_cases && params.use_cases.length > 0) {
            results = results.filter(template => template.metadata?.use_cases?.some(useCase => params.use_cases.includes(useCase)));
        }
        // Apply required services filter
        if (params.required_services && params.required_services.length > 0) {
            results = results.filter(template => template.metadata?.required_services?.some(service => params.required_services.includes(service)));
        }
        // Apply target audience filter
        if (params.target_audience && params.target_audience.length > 0) {
            results = results.filter(template => template.metadata?.target_audience?.some(audience => params.target_audience.includes(audience)));
        }
     ...
     */
    async getTemplateWithWorkflow(templateId) {
    ...
    async listNodeTemplates(nodeTypes, limit = 10, offset = 0) {
        const results = Array.from(this.templates.values()).filter(template => nodeTypes.some(nodeType => template.nodes.includes(nodeType)));
        const total = results.length;
        const items = results.slice(offset, offset + limit);
        return {
            items,
     ...
     */
    async getTemplatesForTask(task, limit = 5) {
     ...
     */
    generateSampleWorkflow(template) {
