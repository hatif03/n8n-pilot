...
export class NodeDiscoveryService {
    ...
    constructor(nodesDir = '../n8n-workflow-builder-mcp-main/workflow_nodes') {
     ...
     */
    findBestMatchingVersion(targetVersion, availableVersions) {
     ...
     */
    async loadNodesForVersion(version) {
    ...
    async searchNodes(params) {
        const { search_term = '', n8n_version, limit = 50, cursor = '0', tags = true, token_logic = 'or' } = params;
     ...
     * Get node categories
     */
    async getNodeCategories(version) {
        const availableVersions = await this.getAvailableVersions();
        ...
        return {
            ...
            subcategories: Object.fromEntries(Object.entries(subcategories).map(([key, value]) => [key, Array.from(value).sort()])),
        };
    }
    /**
     * Get a specific node definition
     */
    async getNodeDefinition(nodeType, version) {
        const availableVersions = await this.getAvailableVersions();
        const targetVersion = version || availableVersions[0];
        const nodes = await this.loadNodesForVersion(targetVersion);
...
//# sourceMappingURL=node-discovery-service.js.map