 ...
 */
export function createEmptyWorkflow(name, description) {
    return {
        id: generateN8nId(),
        name,
        nodes: [],
 ...
 */
export function createWorkflowNode(type, name, position, parameters = {}, typeVersion = 1) {
    return {
        id: generateN8nId(),
        name,
        type,
    ...
    for (const [sourceNodeId, outputs] of Object.entries(workflow.connections)) {
        if (sourceNodeId !== nodeId) {
            ...
            for (const [outputName, connections] of Object.entries(outputs)) {
                updatedConnections[sourceNodeId][outputName] = connections.filter(conn => conn.node !== nodeId);
            }
        }
    }
    return {
 ...
 */
export function addConnectionToWorkflow(workflow, sourceNodeId, sourceOutputName, targetNodeId, targetInputName, targetInputIndex = 0) {
 ...
 */
export function removeConnectionFromWorkflow(workflow, sourceNodeId, sourceOutputName, targetNodeId, targetInputName, targetInputIndex = 0) {
    const connections = { ...workflow.connections };
    if (connections[sourceNodeId] && connections[sourceNodeId][sourceOutputName]) {
        connections[sourceNodeId][sourceOutputName] = connections[sourceNodeId][sourceOutputName].filter(conn => !(conn.node === targetNodeId && conn.type === targetInputName && conn.index === targetInputIndex));
    }
    return {
        ...workflow,
        connections,
 ...
 */
export function findNodeById(workflow, nodeId) {
    return workflow.nodes.find(node => node.id === nodeId);
}
/**
 * Find a node by name in a workflow
 */
export function findNodeByName(workflow, name) {
    return workflow.nodes.find(node => node.name === name);
}
/**
 * Update a node in a workflow
 */
export function updateNodeInWorkflow(workflow, nodeId, updates) {
    return {
        ...workflow,
...
export function validateWorkflow(workflow) {
    ...
    workflow.nodes.forEach((node, index) => {
        ...
        if (!node.name)
            errors.push(`Node ${index}: Name is required`);
        if (!node.type)
...
export function getWorkflowStats(workflow) {
    ...
    const connectionCount = Object.values(workflow.connections).reduce((total, outputs) => total + Object.values(outputs).reduce((outputTotal, connections) => outputTotal + connections.length, 0), 0);
    return {
