...
export class PropertyDependencies {
     ...
     */
    static parseDisplayConditions(conditions) {
        const result = [];
     ...
     */
    static conditionsToObject(conditions) {
        const result = {};
     ...
     */
    static findComplexDependencies(properties) {
     ...
     */
    static findDependentProperties(propertyName, properties) {
        const dependents = [];
        for (const prop of properties) {
     ...
     */
    static propertyInConditions(propertyName, conditions) {
        if (typeof conditions === 'object' && conditions !== null) {
     ...
     */
    static generateSuggestions(dependencies, dependencyGraph) {
        const suggestions = [];
        ...
        if (highDependencyProperties.length > 0) {
            suggestions.push(`Consider grouping these high-impact properties: ${highDependencyProperties.join(', ')}`);
        }
        // Suggest property order
        const orderedProperties = this.topologicalSort(dependencyGraph);
        if (orderedProperties.length > 0) {
            suggestions.push(`Recommended property order: ${orderedProperties.join(' → ')}`);
        }
        ...
        if (requiredProperties.length > 0) {
            suggestions.push(`These properties have dependencies and should be validated: ${requiredProperties.join(', ')}`);
        }
        return suggestions;
    }
    /**
     ...
     */
    static getPropertyDependencies(propertyName, properties) {
     ...
     */
    static validatePropertyConfiguration(propertyName, value, allProperties, currentValues) {
        ...
        if (dependency.enablesProperties && dependency.enablesProperties.length > 0) {
            suggestions.push(`Setting '${propertyName}' will enable these properties: ${dependency.enablesProperties.join(', ')}`);
        }
        // Check if this disables other properties
        if (dependency.disablesProperties && dependency.disablesProperties.length > 0) {
            suggestions.push(`Setting '${propertyName}' will disable these properties: ${dependency.disablesProperties.join(', ')}`);
        }
        return {
     ...
     */
    static checkConditions(conditions, values) {
     ...
     */
    static getDependencyChain(propertyName, properties) {
        const analysis = this.analyze(properties);
        const chain = [];
        const visited = new Set();
        const buildChain = (prop) => {
            if (visited.has(prop))
                return;
            visited.add(prop);
     ...
     */
    static getPropertyGroups(properties) {
        const analysis = this.analyze(properties);
        const groups = [];
        // Group properties by their dependencies
        const processed = new Set();
        for (const dependency of analysis.dependencies) {
            if (processed.has(dependency.property))
                continue;
