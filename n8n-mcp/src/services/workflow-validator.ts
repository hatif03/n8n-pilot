...
import { Workflow, WorkflowNode, WorkflowConnection } from '../types/n8n-api.js';
...
export class WorkflowValidator {
  ...
  private validateConnections(
    ...
    for (const [sourceNodeId, outputs] of Object.entries(connections as Record<string, any>)) {
      ...
      for (const [outputType, connectionsArray] of Object.entries(outputs as Record<string, any>)) {
   ...
   */
  private generateSuggestions(workflow: Workflow, suggestions: string[]): void {
    // Check for common patterns
    const hasWebhook = workflow.nodes.some((n: WorkflowNode) => n.type.includes('webhook'));
    const hasTrigger = workflow.nodes.some((n: WorkflowNode) => n.type.includes('trigger'));
    ...
    const hasErrorHandling = workflow.nodes.some((n: WorkflowNode) => n.onError);
    if (!hasErrorHandling) {
      suggestions.push('Consider adding error handling to your nodes for better reliability');
    }

    // Check for credentials
    const nodesWithCredentials = workflow.nodes.filter((n: WorkflowNode) => n.credentials && Object.keys(n.credentials).length > 0);
    if (nodesWithCredentials.length === 0) {
      suggestions.push('Some nodes may require credentials to function properly');
    }

    // Check for naming conventions
    const unnamedNodes = workflow.nodes.filter((n: WorkflowNode) => !n.name || n.name.trim().length === 0);
