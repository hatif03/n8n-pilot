...
export class ConfidenceScorer {
     ...
     */
    static scoreNodeTypeSuggestion(searchTerm, suggestedNodeType, context) {
        const factors = [];
        let totalWeight = 0;
        let matchedWeight = 0;
     ...
     */
    static scoreWorkflowValidation(workflow, validationResults) {
        const factors = [];
        let totalWeight = 0;
        let matchedWeight = 0;
    ...
    static checkExactFieldMatch(fieldName, nodeType) {
        const knownResourceLocatorFields = {
     ...
     */
    static checkExactNameMatch(searchTerm, nodeType) {
     ...
     */
    static checkPartialNameMatch(searchTerm, nodeType) {
        const nodeName = nodeType.split('.').pop() || '';
     ...
     */
    static checkContextRelevance(context, nodeType) {
        const contextKeywords = context.toLowerCase().split(' ');
        const nodeKeywords = nodeType.toLowerCase().split('.');
        return contextKeywords.some(keyword => nodeKeywords.some(nodeKeyword => nodeKeyword.includes(keyword) || keyword.includes(nodeKeyword)));
    }
    /**
     * Check common usage
     */
    static checkCommonUsage(nodeType) {
        const commonNodes = [
     ...
     */
    static checkCompleteStructure(workflow) {
     ...
     */
    static checkBestPractices(workflow) {
        if (!workflow.nodes)
            return false;
        // Check for descriptive node names
        const hasDescriptiveNames = workflow.nodes.every((node) => node.name && node.name.trim().length > 0);
        // Check for trigger nodes
        const hasTrigger = workflow.nodes.some((node) => node.type.includes('trigger') || node.type.includes('webhook'));
        return hasDescriptiveNames && hasTrigger;
    }
    /**
     * Generate reason based on score and factors
     */
    static generateReason(score, factors) {
