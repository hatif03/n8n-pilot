...
export const composeAiWorkflowTool = {
    ...
    execute: async (args) => {
        try {
            ...
            const nodes = [];
            // 1. Webhook trigger
            const webhookNode = createWorkflowNode('n8n-nodes-base.webhook', 'Webhook Trigger', [100, 100], {
                httpMethod: 'POST',
                path: 'ai-workflow',
            ...
            // 2. AI Agent node
            const agentNode = createWorkflowNode('n8n-nodes-base.openAiAgent', 'AI Agent', [400, 100], {
                model: 'gpt-4',
            ...
            // 3. OpenAI model node
            const modelNode = createWorkflowNode('n8n-nodes-base.openAi', 'OpenAI Model', [400, 300], {
                resource: 'chat',
                operation: 'create',
                model: 'gpt-4',
            ...
            const memoryNode = createWorkflowNode('n8n-nodes-base.memory', 'Memory', [700, 100], {
                operation: 'get',
                key: 'conversation',
            });
            nodes.push(memoryNode);
            // 5. Embeddings node
            const embeddingsNode = createWorkflowNode('n8n-nodes-base.openAiEmbeddings', 'Embeddings', [700, 300], {
                resource: 'embeddings',
                operation: 'create',
            ...
            // 6. Vector store node
            const vectorStoreNode = createWorkflowNode('n8n-nodes-base.pinecone', 'Vector Store', [1000, 100], {
                resource: 'vector',
                operation: 'upsert',
                index: 'ai-workflow',
            ...
            const toolNode = createWorkflowNode('n8n-nodes-base.httpRequest', 'HTTP Tool', [1000, 300], {
                method: 'GET',
            ...
            nodes.push(toolNode);
            // 8. Response node
            const responseNode = createWorkflowNode('n8n-nodes-base.respondToWebhook', 'Response', [1300, 100], {
                respondWith: 'json',
            ...
            const nodeList = nodes.map((node, index) => `${index + 1}. ${node.name} (${node.type})`).join('\n');
...
//# sourceMappingURL=ai-workflow-tools.js.map